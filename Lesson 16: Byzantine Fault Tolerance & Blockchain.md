...
The request processing protocol executes in three phases: pre-prepare, prepare, and commit. When a request is received at the primary, determine based on the view, pick some sequence number, computes the digest of the message, and multicast a pre-prepared request to all of the backup replicas, and also stores this message in a log.
