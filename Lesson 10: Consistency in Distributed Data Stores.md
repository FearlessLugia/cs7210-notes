...
In the COPS solution, when the client tries to read data, it performs a get operation, and this one is served from the local data store, just like in memcache. However, one important difference is that the client library is going to capture information about the fact that this data was read.
