...
The first clock that we will describe is the scalar clock. It was originally proposed by Leslie Lamport, and that's why it's also called Lampard's clock. Those of you who already took advanced operating systems should already be familiar with this clock. As the name suggests, this clock is a scalar. Each node has its own implementation of the clock, which executes the clock rules to produce a new timestamp for the clock. A node only knows the value of the timestamp that it computed, meaning each node may have its own view of the current time, but everyone sees the clock as a single scalar value, and hence it the name.
