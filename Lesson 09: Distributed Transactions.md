...
Each table is replicated, and the replicas are maintained consistent using Paxos. As long as a transaction concerns data in the replicas, Paxos is sufficient.

Even if we think about a single replica set, the system has many ongoing transactions, and there may be multiple that need to access and update values in the same replica set. Because of this, we need some way to control concurrency. We would need that even if we didn't work in a geo-distributed environment. Spanner uses locks, and a mechanism called two-phase locking. This is similar to two-phase commit, but what's being checked by the lock coordinator is whether all participants can also commit to grant the lock, since only then can we consider that the replica set is locked and can avoid any conflict.
